...
安装器把配置创建为安装目录下的 `application.yml`，数据和日志分别位于同目录的 `data\` 与 `logs\`，同一台电脑上的用户共享这一套状态。升级不会覆盖已有配置和数据。普通卸载会弹窗询问是否删除配置、数据和日志，默认选“否”；静默卸载默认保留。默认密钥仅用于本机联调，真实环境必须先修改。MCP 属于 Agent 侧 stdio runtime，不随 Hub 安装。
