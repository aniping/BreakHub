...
产物为 `dist\hub\BreakHub-Setup-0.1.0.exe`。安装界面允许用户选择安装目录，默认目录是 `C:\Program Files\BreakHub`。裁剪后的 Java 17 Runtime 只作为 BreakHub 私有文件放在所选目录的 `runtime\` 下，不安装系统 JDK/JRE，也不修改 `JAVA_HOME` 或 `PATH`。安装、启动、停止和卸载均请求 Windows 管理员权限；手工编辑安装目录下的配置也需要使用管理员权限。卸载项和快捷方式按整机范围创建。为避免误删其他文件，安装器只接受空目录或已有的 BreakHub 安装目录。桌面只创建“BreakHub - 启动”快捷方式；开始菜单提供启动、停止和卸载入口。对应程序是 `BreakHub.exe` 与 `BreakHub-Stop.exe`；启动程序会等待 Spring 服务就绪后自动打开默认浏览器，重复双击会打开正在运行的 Hub 地址，浏览器打开失败时显示可手动访问的地址，Hub 启动失败时则显示诊断日志位置。停止入口通过仅限本机且带随机令牌的控制通道触发 Spring 优雅关闭。

安装器把配置创建为安装目录下的 `application.yml`，数据和日志分别位于同目录的 `data\` 与 `logs\`，同一台电脑上的用户共享这一套状态。同一安装目录的覆盖升级不会覆盖已有配置和数据，已安装后不能在升级时更换目录。此 `0.1.0` 布局不会自动搬运早期开发版放在 `%LOCALAPPDATA%\BreakHub` 的状态；安装器只能检测当前提权账户可见的旧版，因此应在安装前主动卸载任何旧开发版并手工整理数据，避免管理员账户误搬其他用户的文件。普通卸载会弹窗询问是否删除配置、数据和日志，默认选“否”；静默卸载默认保留。默认密钥仅用于本机联调，真实环境必须先修改。MCP 属于 Agent 侧 stdio runtime，不随 Hub 安装。
