...
- `skills/breakpoint-debugging/`：标准 Agent Skill 源目录。
...
- Skill 名和调用方式：`breakpoint-debugging` / `$breakpoint-debugging`；展示名为 `Breakpoint Debugging`。
- MCP 单文件程序：`breakhub-mcp.exe`。
- 目录名：`bp-hub`、`bp-probe`、`bp-mcp`、`skills/breakpoint-debugging`。
- OpenCode MCP 服务键继续使用 `microbreakpoint`，工具前缀 `microbreakpoint_*` 不变。
- 源 Skill 不提交 EXE；打包时才把 EXE 注入包内 `breakpoint-debugging/scripts/mcp/`。
...
- `breakpoint-debugging` 通过官方 Skill 校验，ZIP 只有一个同名顶层 Skill 目录。
