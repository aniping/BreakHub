...
if not defined _BREAKHUB_TASK_SCRIPT (
    echo [BreakHub] Python task script is not configured. 1>&2
    exit /b 2
)
if not defined _BREAKHUB_TASK (
    echo [BreakHub] Python task is not configured. 1>&2
    exit /b 2
)
...
:breakhub_run
"%_BREAKHUB_PYTHON%" "%_BREAKHUB_TASK_SCRIPT%" %_BREAKHUB_TASK% %*
